FUNCTION TSUM2D,X,Y,F               ;Fast trapezoidal summation in 2D
;+
; NAME:
;       TSUM2D
; PURPOSE:
;       Trapezoidal summation of the volume under a surface. By making
;       trapezoidal summation over each line then trapezoidal
...
;       IDL> print, tsum2D(x,y,f), tsumfast(x,cos(x))*tsumfast(y,sin(y))
;             1.1839188       1.1915670
...
yy = y[1:*]-y
ny = n_elements(yy)
bary = (f[1:*,1:*]+f[1:*,*]+f[*,1:*]+f)

sum = total(bary*(xx#transpose(dblarr(ny)+1d))*((dblarr(nx)+1d)#transpose(yy)))/4d
